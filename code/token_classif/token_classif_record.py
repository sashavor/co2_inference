...
_channel = logging.FileHandler('/fsx/sashaluc/logs/token_classif_record.log')
...
def dset_gen():
    dset = load_dataset("super_glue","record", streaming=True, split="test")
...
for model in token_classif_models:
    print(model)
    tracker = EmissionsTracker(project_name=model, measure_power_secs=1, logging_logger=_logger, output_file='/fsx/sashaluc/emissions/token_classif_record.csv')
    tracker.start()
