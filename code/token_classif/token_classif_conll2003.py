...
_channel = logging.FileHandler('/fsx/sashaluc/logs/token_classif_conll2003.log')
...
def dset_gen():
    dset = load_dataset("conll2003", split= 'validation', streaming=True)
...
for model in token_classif_models:
    print(model)
    tracker = EmissionsTracker(project_name=model, measure_power_secs=1, logging_logger=_logger, output_file='/fsx/sashaluc/emissions/token_classif_conll2003.csv')
    tracker.start()
