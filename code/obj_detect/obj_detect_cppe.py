...
_channel = logging.FileHandler('/fsx/sashaluc/logs/obj_detect_testing_cppe.log')
...
def dset_gen():
    dset = load_dataset("cppe-5", split= 'train', streaming=True)
...
for model in obj_detect_models:
    print(model)
    tracker = EmissionsTracker(project_name=model, measure_power_secs=1, logging_logger=_logger, output_file='/fsx/sashaluc/emissions/obj_detection_cppe_resized.csv')
    tracker.start()
