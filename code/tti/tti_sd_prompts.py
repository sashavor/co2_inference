...
_channel = logging.FileHandler('/fsx/sashaluc/logs/tti_testing_sd_prompts.log')
...
def dset_gen():
    dset = load_dataset("Gustavosta/Stable-Diffusion-Prompts", split= 'test', streaming=True)
...
for tti_model in tti_models:
    print(tti_model)
    tracker = EmissionsTracker(project_name=tti_model, measure_power_secs=1, logging_logger=_logger, output_file='/fsx/sashaluc/emissions/tti_sd_prompts.csv')
    tracker.start()
