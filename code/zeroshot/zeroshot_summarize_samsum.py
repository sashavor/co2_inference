...
_logger = logging.getLogger("zeroshot_summarize_samsum")
_channel = logging.FileHandler('/fsx/sashaluc/logs/zeroshot_summarize_samsum.log')
...
def dset_gen():
    dset = load_dataset("samsum", split= 'train', streaming=True)
...
for m in text2text_models:
    print(m)
    tracker = EmissionsTracker(project_name=m, measure_power_secs=1, logging_logger=_logger, output_file='/fsx/sashaluc/emissions/zeroshot_qa_boolq_bloomz.csv')
    tracker.start()
...
for m in text_models:
    print(m)
    tracker = EmissionsTracker(project_name=m, measure_power_secs=1, logging_logger=_logger, output_file='/fsx/sashaluc/emissions/zeroshot_summarize_samsum_bloomq.csv')
    tracker.start()
    tracker.start_task("load model")
