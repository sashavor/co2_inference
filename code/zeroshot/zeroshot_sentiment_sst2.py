...
_logger = logging.getLogger("zeroshot_sentiment_sst2")
_channel = logging.FileHandler('/fsx/sashaluc/logs/zeroshot_sentiment_sst2.log')
...
def dset_gen():
    dset = load_dataset("sst2", split= 'test', streaming=True)
...
for m in text_models:
    print(m)
    tracker = EmissionsTracker(project_name=m, measure_power_secs=1, logging_logger=_logger, output_file='/fsx/sashaluc/emissions/zeroshot_sentiment_sst2_bloomz.csv')
    tracker.start()
    tracker.start_task("load model")
