...
_logger = logging.getLogger("zeroshot_qa_squad2")
_channel = logging.FileHandler('/fsx/sashaluc/logs/zeroshot_qa_squad2.log')
...
for m in text_models:
    print(m)
    tracker = EmissionsTracker(project_name=m, measure_power_secs=1, logging_logger=_logger, output_file='/fsx/sashaluc/emissions/zeroshot_qa_squad2_bloomz.csv')
    tracker.start()
    tracker.start_task("load model")
    pipe = pipeline("text-generation", model=m, device=0, model_kwargs={"force_download":True}, max_new_tokens = 65, trust_remote_code=True)
    model_emissions = tracker.stop_task()
    count=0
    tracker.start_task("query model")
    for p in qa_df:
