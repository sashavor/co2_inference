...
_channel = logging.FileHandler('/fsx/sashaluc/logs/image_class_testing_cifar10.log')
...
def dset_gen():
    dset = load_dataset("cifar10", streaming=True, split="test")
...
for model in im_classif_models:
    print(model)
    tracker = EmissionsTracker(project_name=model, measure_power_secs=1, logging_logger=_logger, output_file='/fsx/sashaluc/emissions/image_class_cifar10_resized.csv')
    tracker.start()
