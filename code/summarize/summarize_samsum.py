...
_channel = logging.FileHandler('/fsx/sashaluc/logs/summarize_testing_samsum_final.log')
...
for sum_model in summarization_models:
    print(sum_model)
    tracker = EmissionsTracker(project_name=sum_model, measure_power_secs=1, logging_logger=_logger, output_file='/fsx/sashaluc/emissions/summarize_samsum_15.csv')
    tracker.start()
    ...
    count=0
    for d in dataset:
        summarize(d['dialogue'], max_length= 15, min_length=10)
    print('================'+str(count)+'================')
    model_emissions = tracker.stop_task()
    _ = tracker.stop()
